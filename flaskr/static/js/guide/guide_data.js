...
var tip_step_6 =
[
            { element : '#img_results_div', tooltip : 'Uni-variate selection ranks the genes individually, from the most to the least informative. By examining the knee-point of this graph, we can see the number of genes that shows  significant difference !', text: 'Uni-variate selection', position: 'R' },
            { element : '.use-download-univarient_img', tooltip : 'Download this graph !', text: 'Download image', position: 'T' },
            { element : '#table_results_div', tooltip : 'Classification is the process of predicting the class of given data points. A classifier is a function that is used to assign the samples to a class. Classification accuracy of the selected genes using 5 classifiers is shown in the table !', text: 'Classification results', position: 'L' },
            { element : '#select_num_features_div', tooltip : 'The number of features can be reduced by examining the knee-point of the uni-variate selection graph. Select the feature count to proceed to next steps !', text: 'Select feature count', position: 'T' }
];

var tip_fs_index =
[
{ element : '#selected_file_div', tooltip : 'This is the file you will have after going through previous steps. It will be automatically selected. You do not need to change anything, if you wish to proceed with the same file !', text: 'Selected file', position: 'T' },
            { element : '#change_file_div', tooltip : 'You can also perform feature selection on a separate file without going through the pre-processing steps. Use this feature if you want to select a new file !', text: 'Change file', position: 'T' },
            { element : '#select_methods_div', tooltip : 'Feature selection is the process of selecting the features that contribute mostly to the target variable.  Different feature selection methods are used for this purpose. In this application, you may select 3 feature selection methods to get the best set of features !', text: 'Feature Selection Methods', position: 'T' },
            { element : '#num_of_features', tooltip : 'Specify the number of features you want to filter out by the above feature selection methods !', text: 'Number of Features Selected', position: 'T' }
];
...
[
            { element : '#results_plot_div1', tooltip : 'Variation of correlation values with the number of correlated features by considering all the 3 methods !', text: 'Correlated features', position: 'T' },
            { element : '#results_plot_div2', tooltip : 'Variation of correlation coefficient with number of features and the variation of correlation coefficients with classification accuracy !', text: 'Variation of the correlation coefficients', position: 'T' },
            { element : '#results_table_div', tooltip : 'Filtering out the least correlated features (to minimize redundancy) with the highest classification accuracy !', text: 'Correlation matrix', position: 'T' },
            { element : '#overlapped_features_div', tooltip : 'You can view the gene set that was selected by all three feature selection methods. This is the overlap set of genes !', text: 'Overlapped features', position: 'T' },
            { element : '#correlation_features_div', tooltip : 'You can view the gene set that was obtained by considering the correlation coefficients and the highest accuracy !', text: 'Correlation | {{ max_clasify }}', position: 'T' }
];

var tip_an_final =
[
            { element : '#gene_output_div1', tooltip : 'This shows the set of genes selected by considering the least correlation values and the highest accuracy !', text: 'Extracted genes', position: 'T' },
            { element : '#results_plot_div1', tooltip : 'ROC curves and the correlation matrix of the selected genes !', text: 'ROC and Correlation matrix', position: 'T' },
            { element : '#results_table_div1', tooltip : 'This table shows the testing and training accuracies of the above selected genes !', text: 'Accuracy results', position: 'T' },
            { element : '#gene_output_div2', tooltip : 'This shows the set of genes selected by considering the least correlation values and the highest accuracy !!', text: 'Extracted genes', position: 'T' },
            { element : '#results_plot_div2', tooltip : 'ROC curves and the correlation matrix of the selected genes !', text: 'ROC and Correlation matrix', position: 'T' },
            { element : '#results_table_div2', tooltip : 'This table shows the testing and training accuracies of all the selected features. This is the final accuracy of the  best set of genes !', text: 'Accuracy results', position: 'T' }
];

var tip_validation =
[
            { element : '#info_results_div', tooltip : 'Now, we need to validate our results to confirm their biological relevance. For this, we use data from https://www.genecards.org/ to compare our results. In GeneCards, they have information on the already identified risk genes for a particular disease!', text: 'Validation', position: 'T' },
            { element : '#results_table_div1', tooltip : 'This table gives information on the already identified genes !', text: 'Validation', position: 'T' }
];

var tip_model_index =
[
            { element : '#extracted_features_div', tooltip : 'These are the final set of features that were extracted from the previous steps !', text: 'Extracted features', position: 'T' },
            { element : '#choose_file_div', tooltip : 'Choose a final containing the training data to be used in modelling !', text: 'Choose file ', position: 'T' },
            { element : '#classifier_method_div', tooltip : 'Select a classifier to proceed with developing the model !', text: 'Classifier', position: 'T' }
];

var tip_model_predict =
[
            { element : '#modal_create', tooltip : 'The process of modeling means training a machine learning algorithm to predict the labels from the features. The output from modeling is a trained model that can be used for making predictions on new data sets !', text: 'Custmize model', position: 'T' },
            { element : '#select_file_div', tooltip : 'Choose the file that you want to predict the target output !', text: 'Choose File ', position: 'T' },
            { element : '#select_normalize_div', tooltip : 'You can choose to perform the normalization on the selected data set too by turning this feature on !', text: 'Normalize method', position: 'T' },
            { element : '#select_annotation_div', tooltip : 'You can map the probe ids to gene symbols by performing annotation table mapping. Turn on this feature to proceed with table mapping !', text: 'Annotation mapping', position: 'T' }
];
